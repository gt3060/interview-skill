<template>
    <div class="learnVue">
            ...
            <div class="learnCardContain">
                ...
                <div class="titleStyle">
                    <div class="titleIconStyle">
                        <img src="@/components/picture/vue.jpeg"
                             class="imgStyle" />
                    </div>
        ...
        const object1 = {};<br/>
            Object.defineProperty(object1, 'property1', {<br/>
                value: 42,<br/>
                writable: false,<br/>
            });<br/>
            ...
            console.log(object1.property1);//42
                            ...
                            <div id="c4"
                                ...
                                <div class="fontIndent">
                                    <p>
                                        在style标签中写入scoped即可，例如：<code>&lt;style scoped &gt;&lt;/style&gt;</code>
                                    </p>
                                </div>
                            </div>
                            <text-field catalog
                                        ...
                                        content="v-if按照条件是否渲染手动添加或删除dom元素，v-show是display的block或none；"></text-field>
                            <text-field catalog
                                        id="c6"
                                        title="vue的两个核心是什么？"
                                        content="数据驱动和组件系统"></text-field>
                            <text-field catalog
                                        id="c7"
                                        title="vue常用的指令有哪些？"
                                        content="v-on（@）、v-if、v-show、v-for、v-bind（：）、v-else"></text-field>
                            <div id="c8"
                                 style="margin: 20px 0px">
                                <div style="display: flex">
                                    <div style="margin: 6px 7px 0px 0px">
                            ...
                            <text-field catalog
                                        id="c9"
                                        title="vue中key值的作用"
                                        content="当vue利用v-for正在更新已渲染的元素列表时，它默认用“就地复用”策略，如果数据项的顺序改变，vue将不会移动dom元素，
                                        来匹配数据项的顺序。而是简单复用此处每个元素，并且确保它在特定索引下显示已被渲染过的每个元素，key的作用就是为了高效的更新
                                        虚拟dom。"></text-field>
                            <text-field catalog
                                        id="c10"
                                        title="什么是vue的计算属性？"
                                        content="在模板中放入太多的逻辑会让模板过于沉重且难以维护，在需要对数据进行复杂处理，且可能多次使用情况下，尽量采取计算属性的方式，
                                        ...
                                        都会重新计算，但是依赖数据不变的时候computed从缓存中获取，不会重新计算。"></text-field>
                            <div id="c11"
                                 style="margin: 20px 0px">
                                <div style="display: flex">
                                    <div style="margin: 6px 7px 0px 0px">
                            ...
                            <text-field catalog
                                        id="c14"
                                        title="简单说一下vue2.x响应式数据原理"
                                        content="vue在初始化数据时，会使用Object.defineProperty重新定义data中的所有属性，当页面使用对应属性时，
                                        首先会进行依赖收集（收集当前组件的watcher），如果属性发生变化会通知相关依赖进行更新操作（发布订阅）。"></text-field>
                            <div id="c15"
                                 style="margin: 20px 0px">
                                <div style="display: flex">
                                    <div style="margin: 6px 7px 0px 0px">
    ...
    data() {
        return {
            catalogHtmlData: [
                ...
                {
                    index: 'c14',
                    name: '15.简单说一下vue2.x响应式数据原理',
                    i: '14',
                },
                {
                    index: 'c15',
                    name: '16.简单说一下vue3.x响应式数据原理',
                    i: '15',
                },
